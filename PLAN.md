...
| Slack bot, leasing | **Not built.** Same permissions graph will apply when they exist. |

**Channel membership is done** (2026-08-19) — a desk/DM now reads the private rooms the actor is actually in, matched by email. **Personal connectors are done** (2026-08-20, gmail/slack) — see CP 8b's status note. Remaining skeleton checkpoints: **Slack surface** then **MCP**, both constructing `AskContext` server-side.
...
- [ ] Slack bot surface constructs AskContext from the event — **not built** (§17 cut list: web desk must keep working, this is lowest priority)

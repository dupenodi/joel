...
| Ingest | Ten connectors, lookback re-fetch (+ a real backward-walking deep-backfill pass for slack/gmail, §11.3), content_hash triage, background scheduler (concurrency-capped, errored connectors retry on a backoff ladder honouring real `Retry-After`), zombie-job reclaim on boot. Personal connectors (gmail/slack) real: `UNIQUE(provider, owned_by)`, personal-then-org-shared read resolution. |
...
| MCP, API keys | **Built (2026-08-20).** `/mcp/` (Streamable HTTP), one `ask` tool, API-key auth (`api_keys` table, migration 009) mapping to a normal Actor — same `AskContext`/`answer_question` path as web, built server-side from the key's owner. See CP 8b's status note. |
| Slack bot | **Built (2026-08-20).** `POST /api/slack/events`, real signature verification, `app_mention` -> AskContext built server-side from the mentioning Slack user's own email match -> a real reply posted in-thread. See CP 8b's status note. |
| Leasing | **Not built.** Same permissions graph will apply when it exists. |

**Channel membership is done** (2026-08-19) — a desk/DM now reads the private rooms the actor is actually in, matched by email. **Personal connectors, the MCP surface, and the Slack bot surface are all done** (2026-08-20) — see CP 8b's status note. Every skeleton checkpoint on this list is now real; access leasing (§17's lowest-priority cut) is the one remaining item, and it stays cut.
...
**⚠️ Superseded by the Composio-broker decision — see CP 8's 2026-08-20 status note before reading this as a joel TODO.** Composio holds and refreshes every provider token; joel never sees a raw `access_token`/`refresh_token` to store or refresh. The table below is what Composio does on joel's behalf, not code joel needs to write.
...
**(2026-08-20) §11.4 corrected — it described a design that was superseded before it shipped.** §11.4 as written says joel stores `access_token`/`refresh_token`/`expires_at` Fernet-encrypted and refreshes on 401 itself. That was true of the original OAuth-direct plan; it stopped being true the moment every connector moved behind Composio as the auth broker (`connectors/composio_conn.py`'s own docstring: *"Composio masks access tokens on connected-account reads"*). `connector_credentials.encrypted_json` today holds only a `composio_account_id` reference — no provider ever hands joel a raw token to refresh. Composio owns the entire refresh lifecycle server-side; every provider call goes through `composio.tools.proxy(connected_account_id=...)`, never a token joel holds. A 401/403 coming back through that proxy therefore means Composio itself couldn't refresh (the user revoked access, or the refresh token itself is dead) — `needs_reauth` is the objectively correct response in that case, not a bug to fix. Writing joel-side refresh code now would be dead code duplicating a responsibility Composio already owns correctly. §11.4's table is left below for historical context but should be read as "what Composio does for us," not a joel TODO.

**Fixed this pass:** Composio proxy 429 backoff now honours the real `Retry-After` header (seconds or HTTP-date, case-insensitive, capped at 30s so a misbehaving header can't block a sync worker indefinitely) instead of a fixed `1+attempt` guess — `connectors/composio_conn.py::_retry_after_seconds`, `check_8_sync.py::check_retry_after_is_honoured_exactly`.

**Still real, not attempted this pass:** Progressive deep backfill (§11.3's separate backward-walking pass) doesn't exist — every sync is still the lookback-window re-fetch §6/§16.2 already documented as the accepted design instead of cursors, so "crash resume" is really "the next lookback re-fetch is idempotent via content_hash," not a literal cursor resume.
...
**Status 2026-08-20 — stamp + read table + channel membership + personal connectors + the MCP surface all done.** Leasing and the Slack bot remain not built. `scripts/check_visibility.py` green (`check_channel_membership`), `scripts/check_personal_connectors.py` green, `scripts/check_mcp_server.py` (new) green.

**MCP surface — real, not a stub.** §0.3's "API keys — not built" was the literal blocker: MCP clients aren't browser sessions with cookies, so they need their own identity mechanism. New `api_keys` table (migration 009) maps a key to exactly one person — a request made with it gets that person's own normal permissions through the exact same `AskContext`/`answer_question` path every other surface uses, no separate privilege model to keep in sync. `identity.py::create_api_key`/`list_api_keys`/`revoke_api_key`/`actor_from_api_key` mirror the invite-token pattern already there (SHA-256 hash stored, raw key shown exactly once). A real "API keys" panel lives in Settings (`web/components/api-keys-panel.tsx`) — create, reveal-once, list with last-4 + last-used, revoke — verified live in a real browser end to end.

New `joel/mcp_server.py` wraps the official `mcp` SDK's `MCPServer` (Streamable HTTP transport) with joel's own bearer-auth ASGI middleware — deliberately NOT the SDK's `token_verifier`/`AuthSettings` machinery, which is built for full OAuth dynamic client registration and a `.well-known` discovery surface; a static per-person API key needs neither, just "whose key is this," so auth happens entirely outside that system via a raw ASGI wrapper (not `BaseHTTPMiddleware`, so the transport's streaming responses pass through unbuffered) and a plain `contextvars.ContextVar`. One tool, `ask`, reuses the exact same `answer_question` retrieval/synthesis pipeline `/api/ask` calls — honestly scoped to standalone questions only for now, not (yet) wrapped in the follow-up-rewriting or live-lookup layers `/api/ask` adds around it, same "N of M implemented, extend later" shape as CP10's live lookup.

Two real integration snags, both fixed: (1) `app.mount()` does not chain a sub-app's `lifespan` into the outer app's, so the transport's task group was never started — `RuntimeError: Task group is not initialized` on every real call. Fixed by starting/stopping the session manager's own `run()` context manager directly from `_startup`/`shutdown` hooks, kept separate from the app's existing sync `_startup()` rather than restructuring it around FastAPI's `lifespan=` parameter. (2) `streamable_http_app()` registers its own internal route at `/mcp` by default, which combined with mounting the whole app at `/mcp` in the outer app made the only real reachable path `/mcp/mcp`, 500ing with the task-group error above until noticed — fixed with `streamable_http_path="/"`.

Verified with a genuine MCP client (the real `mcp` SDK, not a hand-rolled request) against the real running dev server: `initialize` handshake, `list_tools` → `["ask"]`, and a real `call_tool("ask", ...)` returned a real, grounded, correctly-cited answer from the real corpus ("is amarpathak/vaadi#1 merged?" → "No, ... was not merged. It was closed without merging."); a request with no bearer token, and one with a wrong key, were both correctly rejected. `scripts/check_mcp_server.py` covers the API-key lifecycle (create/list/resolve/revoke, revoke scoped to its owner, a revoked key stops resolving immediately) and the bearer-auth ASGI wrapper directly (fake scope/send, no real server needed) as fast synthetic checks, plus the one real live round-trip check (skips honestly if no dev server is reachable).
...
**Connector ownership — real now (2026-08-20), not just groundwork.** Migration `006` added `connections.owned_by`/`connections.kind` (`org`|`personal`) but was explicitly groundwork-only at the time: `connections.provider` still had a `UNIQUE` constraint, so there was no way to have more than one connection per provider at all, period. Migration `008_personal_connectors.sql` rebuilds the table (SQLite can't ALTER a UNIQUE constraint in place) with `UNIQUE(provider, owned_by)` instead — SQLite treats every `NULL` as distinct from every other `NULL`, so this allows exactly one org-shared row (`owned_by IS NULL`) *plus* one personal row per user, per provider, and every existing row (all `NULL`-owned) stays valid unchanged. Verified against a real copy of the populated production DB (1461 docs, 5 connections, byte-identical before/after) before ever touching the real one.

Scoped to mailbox/DM-shaped providers only — `PERSONAL_CONNECTOR_PROVIDERS = {gmail, slack}` in `app.py`, checked server-side (never trust the client for a scope decision): a second Notion/Drive connection wouldn't mean anything different from the org-shared one, but a second Gmail inbox or Slack identity is naturally one person's. Every call site that assumed one row per provider is fixed: `_upsert_connection` now keys its existing-row lookup on `(provider, owned_by)`; `/api/connectors` (`_connectors_for_actor`, a pure function so it's directly unit-tested) shows each actor their own personal row over the org one and *never* another actor's personal row; `_run_live_lookup`'s credential resolution now prefers the asking actor's own personal connection over the org-shared one (`ORDER BY (owned_by IS NULL)` — a one-line real instance of Supermemory's personal-then-org-shared read model, not just schema groundwork for it). The connect flow: since the OAuth callback is a public, unauthenticated redirect endpoint with no session to consult, "connect as personal" has to survive the round trip through `pending_connects` (new `owned_by` column, same pattern as the `lookback_days`/`return_to`/`origin` fields already crossing that same gap) rather than being read off the request that eventually lands back. `web/components/integrations/integration-modal.tsx` offers a real "Just for me" checkbox, gated to gmail/slack and to the not-yet-connected state.

Verified: `scripts/check_personal_connectors.py` (schema allows org+personal to coexist while true duplicates still reject; `_upsert_connection` idempotent per owner; the three-actor `_connectors_for_actor` view; live-lookup credential preference) plus a real end-to-end hit on the live server — `POST /api/composio/connect {toolkit:"slack", personal:true}` correctly wrote `owned_by` into `pending_connects`, and the same call for `notion` correctly 400'd ("Notion can't be connected as a personal connector"). Not driven through a full real OAuth round-trip end-to-end (both gmail and slack are already org-connected on this install; disconnecting either to free up a personal-connect demo would touch live production data for a demo, not a real need) — the callback-side wiring (`_activate_composio_toolkit` → `_upsert_connection` with `owned_by`) is covered by `check_personal_connectors.py` instead.

**Slack bot — the last item, real, not a stub (2026-08-20).** New `joel/slack_bot.py`: `verify_signature` (Slack's documented `v0=HMAC-SHA256(secret, "v0:{ts}:{body}")` scheme, constant-time compared, plus the 5-minute replay window Slack's own docs specify), `parse_app_mention` (a real, well-formed `app_mention` or `None` — never a guess on anything else, including the bot's own messages, which lack a real `user` field), `strip_mention` (removes only the bot's own leading `<@BOTID>`, leaves a mention of anyone else alone since that's real question content), and `SeenEvents` (a small bounded in-memory dedupe set — Slack retries a delivery that didn't get a fast-enough 200, and answering twice would double-post).

`POST /api/slack/events` is public (added to `_is_public_path`) — it authenticates itself via the real Slack signature, never the `joel_session` cookie, and handles the one-time `url_verification` handshake Slack does when the Events API URL is first saved. Slack needs a response within 3 seconds; real retrieval is real LLM calls, seconds not milliseconds, so the route returns immediately and the actual work (`_handle_slack_mention`) runs on a background thread, same pattern as `_start_ingest`. Actor resolution reuses §1.4's email-matching signal a third time (after Gmail visibility and channel membership): the mentioning Slack user's profile email is matched against `users.email` via `identity.actor_for_user` (new, a thin public wrapper around the same lookup `actor_from_session`/`actor_from_api_key` already use internally) — no match means the mention is silently declined, never answered with an org-wide `AskContext` for someone unrecognized. A recognized mention reuses `_mcp_ask_sync` (the same one the MCP surface calls) for the actual answer, then replies in-thread via `chat.postMessage` through the existing Composio-proxied Slack caller (confirmed live, with the user's explicit approval before sending any real message, that `chat.postMessage` works through this codebase's existing GET-based proxy path — the same one `conversations.history`/`user_emails`/etc. already use).

Verified: `scripts/check_slack_bot.py`'s six synthetic checks (signature accept/reject/replay, real event parsing including url_verification and malformed-event rejection, mention-stripping, dedupe) plus two real checks — (1) the real running server's real `/api/slack/events` route, hit over real HTTP with a real settings-backed signing secret: a correctly-signed request is accepted and correctly echoes the `url_verification` challenge, a wrongly-signed one gets a real 401; (2) a full real happy path run in-process against the real corpus and the real connected Slack workspace, with only the one piece that can't be arranged for real on this install faked (this install's real admin email doesn't match any real connected Slack member's, the same gap CP8b's channel-membership work already found) — everything downstream of that is real: a real `answer_question` call and a real `chat.postMessage` reply that actually landed in the real Slack channel, confirmed by diffing `conversations.history` before/after.
...
- [x] **(2026-08-20)** org-shared vs personal connector scope — real: `UNIQUE(provider, owned_by)`, personal-then-org-shared read resolution in live lookup, `/api/connectors`'s per-actor view, a real "Just for me" connect flow for gmail/slack
- [x] **(2026-08-20)** MCP surface constructs AskContext from the event — real: `mcp_server.py`'s `ask` tool builds it server-side from the API key's owner, never from anything the MCP client sends
- [x] **(2026-08-20)** Slack bot surface constructs AskContext from the event — real: `slack_bot.py`'s mention handler resolves the Actor server-side from the Slack user's own email match, never from anything in the event payload
...
- [x] **(2026-08-20)** `Message.tool_calls` is real, not always `[]` — `_assistant_payload`'s `tool_calls` param was defined but every call site left it at its `[]` default, so the field existed in the response schema but never carried data; the frontend had no renderer for it either (`ToolChips.tsx` is a hardcoded dev-gallery demo, not wired to real data). §13.2's live lookups are the agent's only tool calls today, so `/api/ask` now builds one `ToolCall` per checked live target (`id`/`name="live_lookup"`/`provider`/`status`/`detail`), emits it as its own `tool_call` SSE event, and includes it on the final message; new `web/components/beautifului/ToolCallChips.tsx` (real, minimal, matches `LaneChips`' visual weight) renders it in `AnswerTurn`. Verified live over raw SSE and against `check_10_agent.py`/`check_7_retrieve.py`.
...
- [ ] 👁 all four status badges render correctly — `Answered` (green) and `Not in memory`/`absent` (grey, via `AbsentAnswer`) both confirmed live 2026-08-20; `partial`/`conflicted` (orange) still need a real superseded/conflicting pair in the corpus to trigger honestly, not yet exercised
...
- [x] **(2026-08-20)** `/settings` visual rebuild — it was the one page that never adopted this app's card language: `workspace-panel.tsx`/`api-keys-panel.tsx`/`install-panel.tsx` rendered flat `<section>` blocks with no card boundary, ghost-only (`variant="secondary"`) buttons that read as unstyled dead text next to inputs, and plain-text role/role-tag labels — all visibly inconsistent next to `/integrations` and `/graph`, which already use `rounded-card bg-surface shadow-card` tiles, `primary`/`accent`/`danger` buttons, and pill badges. Rewrote all three components (plus `SettingsSkeleton` to match) onto that same convention: every logical group is now a `rounded-card bg-surface p-5 shadow-card` section, member/invite/key lists are edge-to-edge bleed rows inside the card, role/"you"/"Connected" indicators are pill badges matching `ConnectorStatus`'s style, and buttons follow the rest of the app's hierarchy (`primary` for in-card saves, `accent` for the one main CTA per card, `danger` for destructive rows). Also fixed a real contrast bug found live: the enabled "Wipe" button was `bg-red-tint text-red` sitting on a `bg-red-tint` card, making it nearly invisible — now solid `bg-red`/white text when enabled. Verified live in a real browser (Playwright) against the real running dev server: card layout, all four save flows, invite, a real API-key create→reveal→list→revoke round trip, and the wipe-button contrast fix. No other page audited this pass needed the same fix — `/integrations`, `/graph`, `/` (chat), and the auth screens (`/login`/`/setup`/`/join`, via the shared `AuthScreen` component) were all already on-convention.

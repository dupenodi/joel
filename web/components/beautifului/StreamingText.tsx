...
const ABSENT_TOKENS: Token[] = [
  ..."Not in the company's memory.".split(" ").map((text) => ({ text })),
];
...
const SOURCES = [
  { name: "#billing-owners", provider: "slack", href: "https://slack.com/" },
  {
    name: "RFC: billing ownership",
    provider: "github",
    href: "https://github.com/",
  },
  {
    name: "Q3 reversal note",
    provider: "gmail",
    href: "https://mail.google.com/",
  },
];

function SourceChip({ provider }: { provider: string }) {
  return (
    <a
      href={SOURCES.find((s) => s.provider === provider)?.href ?? "#"}
      target="_blank"
      rel="noreferrer"
    ...
    >
      <SourceIcon provider={provider} size={12} className="rounded-[3px]" />
      <span>{provider}</span>
    </a>
  );
}
...
export default function StreamingText({
  ...
  fill?: boolean;
  /** short “not in memory” answer with no sources */
  absent?: boolean;
  onDone?: () => void;
}) {
  const tokens = absent ? ABSENT_TOKENS : DEFAULT_TOKENS;
  const sources = absent ? [] : SOURCES;
    ...
    <div className={fill ? "w-full" : "min-h-[15.5rem] w-full max-w-95"}>
      <p className="text-[13px] leading-relaxed text-ink">
        {tokens.slice(0, count).map((token, i) =>
          token.cite ? (
            <SourceChip key={i} provider="slack" />
          ) : (
            <span
              key={i}
